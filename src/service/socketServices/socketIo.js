...
function mySocket(server) {
  ...
  io.use(async (socket, next) => {
    ...
    const getBlockedContactsID = await blockedIDs(isValidToken.userData.userID);

    if (isValidToken?.success && getBlockedContactsID?.success) {
      let user = isValidToken.userData;
      userInfo[user.userID] = {
        user,
        timestamp: isValidToken?.timestamp,
        visitCounter: isValidToken?.visitCounter,
      };
